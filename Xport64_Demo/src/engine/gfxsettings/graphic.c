...
{
    // Clear the Z-buffer  
		//gDPSetScissor(glistp++,G_SC_ODD_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT); //interlace for graphics...? Might work for some graphical effects but does not improve performance
		gDPSetScissor(glistp++,G_SC_NON_INTERLACE, 0,0, CLEAR_WD,CLEAR_HT); //non-interlace for black
    gDPSetDepthImage(glistp++, OS_K0_TO_PHYSICAL(nuGfxZBuffer));
    gDPPipeSync(glistp++);
    gDPSetCycleType(glistp++, G_CYC_FILL);
    gDPSetColorImage(glistp++, G_IM_FMT_RGBA, G_IM_SIZ_16b,320,
		     OS_K0_TO_PHYSICAL(nuGfxZBuffer));
    gDPSetFillColor(glistp++,(GPACK_ZDZ(G_MAXFBZ,0) << 16 |
			      GPACK_ZDZ(G_MAXFBZ,0)));
    gDPScisFillRectangle(glistp++, 80, 10, CLEAR_WD-1-80, SCREEN_HT-1);
	...
	//gDPSetScissor(glistp++,G_SC_NON_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT); //non-interlace for black
	//BLACK FRAME BUFFER
    gDPSetFillColor(glistp++, (GPACK_RGBA5551(0, 0, 0, 1) << 16 | 
			       GPACK_RGBA5551(0, 0, 0, 1)));
				   
   // gDPFillRectangle(glistp++, 0, 0, CLEAR_WD-1, CLEAR_HT-1);
	gDPScisFillRectangle(glistp++, 0, 0, CLEAR_WD-1, SCREEN_HT-1);
...
{
	gDPSetScissor(glistp++,G_SC_ODD_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT);
...
{ 
		...
		//TEXTURE_FOG_VTXCOL(glistp, 125, 125, 95, 0,990,1000); //call only once for all similar env objects
		

		
		//TO DO NOTE: Figure out why graphics not rendering correctly on console

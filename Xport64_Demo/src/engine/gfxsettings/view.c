...
//ADJUSTABLE CAMERA ANGLE FIXED----------------------------------------------------------------------------------------------------
	...
	guPerspective(&dp->projection, &perspNorm, 40, ((float)ANAMORPHIC_WD/(float)ANAMORPHIC_HT), nearPosition, 5750, 1.0f);
...
/*------------------------Calculate Rotating Camera-------------------------*/
		...
		//lerp(mainCamera.obj.pos.z,camUpdatedPosition.z,(float)GetDeltaTime()*(cameraSmoothSpeed*10.0f)
		
		//camera lookObject point is smoothed on the z axis to keep from juttery camera movements
		smoothLookZ = lerp(mainCamera.camLookPoint.z, playerAvatar.obj.pos.z+5.0f,(float)GetDeltaTime()*(cameraSmoothSpeed*0.75f)); 
		
		//SetVector3(&mainCamera.camLookPoint, playerAvatar.obj.pos.x, playerAvatar.obj.pos.y, playerAvatar.obj.pos.z + 5.0f); //Note: distance to offset from "camLook"
		SetVector3(&mainCamera.camLookPoint, playerAvatar.obj.pos.x, playerAvatar.obj.pos.y, smoothLookZ); //Note: distance to offset from "camLook"
